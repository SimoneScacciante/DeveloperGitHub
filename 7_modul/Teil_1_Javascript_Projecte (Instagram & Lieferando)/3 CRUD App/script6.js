...
function load() {
    let nameAsText = localstorage.getItem('namesListe')  //nun "getItem" ((???verstehe nicht???))
    let phonenumberAsText = localstorage.getItem('phoneNumbersListe')
    
    if (namesAsText && phonenumberAsText) { // mit "if" abfrage sagst du... wenn beide 
                                           // variablen existieren, dann führe Code aus
                                     // da es sein kann das eins von beiden Array Leer ist
    names = JSON.parse(namesAsText); // parse = Text wird wieder zu Array umgewandelt
